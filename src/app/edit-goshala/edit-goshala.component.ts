...
private resetFormFields(fields: string[]) {
  ...
  onSubmit(): void {
    ...
    const goshalaId = this.route.snapshot.paramMap.get('_id');
    console.log('Goshala ID:', goshalaId);
    console.log('Goshala Data:', this.goshalaData);
  
    if (!goshalaId) {
      console.error('Error: Goshala ID is missing from route.');
      return;
    }
  
    if (!this.goshalaData) {
      console.error('Error: Goshala Data is missing or not initialized.');
      return;
    }
